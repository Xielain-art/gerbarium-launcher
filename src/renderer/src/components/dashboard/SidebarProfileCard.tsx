...
export function SidebarProfileCard({ t, user, onOpenSettings, onLogout }: Props) {
  const roleLabels = getRoleLabels(user);

  return (
    <ShadcnCard className="border-white/10 bg-black/20 p-3">
      <div className="flex items-center gap-3">
        <div className="shrink-0 py-1 pl-1"><Avatar username={user?.username} size="lg" /></div>
        <div className="min-w-0 flex-1">
          <span className="block truncate font-minecraft text-sm font-bold text-theme" title={user?.username || ""}>{user?.username || t.DASHBOARD.PLAYER_DEFAULT}</span>
          <div className="mt-0.5 font-minecraft text-xs text-theme-muted">{t.DASHBOARD.PLAYER_ID_LABEL} {user?.id?.slice(0, 8) || t.DASHBOARD.PLAYER_ID_UNKNOWN}</div>
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {roleLabels.map((role) => <ShadcnBadge key={role} variant="secondary" className="border-0 bg-[var(--mc-accent)] px-2 py-0.5 font-minecraft text-[10px] font-bold text-white">{role}</ShadcnBadge>)}
          </div>
        </div>
