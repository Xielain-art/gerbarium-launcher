    ...
    <div className="grid gap-5">
      {news.map((item) => (
        <Card key={item.id} className="overflow-hidden p-0">
          ...
          <p className="font-minecraft text-sm leading-relaxed text-theme-muted">{(item.htmlContent ? stripHtml(item.htmlContent) : item.content).slice(0, 260)}{(item.htmlContent ? stripHtml(item.htmlContent) : item.content).length > 260 ? "..." : ""}</p>
          <div className="mt-4"><button type="button" className="mc-btn mc-btn-sm mc-btn-primary" onClick={() => onSelectNews(item)}>Read more</button></div>
        </Card>
      ))}
    </div>
  );
