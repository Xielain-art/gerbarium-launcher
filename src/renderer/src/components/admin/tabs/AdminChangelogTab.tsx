      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2 border-b border-white/10 pb-2">
          ...
          <div className="ml-auto">
            ...
            >
              {p.editingChangelog ? "✏️ Редактировать версию" : "➕ Создать версию"}
            </ShadcnButton>
          </div>
        </div>
      </div>
      ...
      {p.changelogTab === "all" && (
        <>
          <div className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-3">
            <ShadcnInput label="От даты" type="datetime-local" value={p.changelogFromDate} onChange={(e) => p.setChangelogFromDate(e.target.value)} />
            <ShadcnInput label="До даты" type="datetime-local" value={p.changelogToDate} onChange={(e) => p.setChangelogToDate(e.target.value)} />
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            <ShadcnSelect label="Сортировка" value={p.changelogSortDraft} onChange={(e) => p.setChangelogSortDraft(typeof e === "string" ? e : e.target.value)} options={[{ label: "Release", value: "releaseDate" }, { label: "Version", value: "version" }, { label: "Created", value: "createdAt" }]} />
            <ShadcnSelect label="Порядок" value={p.changelogOrderDraft} onChange={(e) => p.setChangelogOrderDraft(typeof e === "string" ? e : e.target.value)} options={[{ label: "Новые", value: "DESC" }, { label: "Старые", value: "ASC" }]} />
            <div></div>
          </div>
      ...
      {p.changelogTab === "create" && (
        <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-minecraft text-base font-bold text-theme">
                  {p.editingChangelog ? `Режим редактирования: v${p.editingChangelog.version}` : "Режим создания версии"}
                </h3>
          ...
          <div className="space-y-4 rounded-lg border border-white/10 bg-black/10 p-6">
            <h4 className="font-minecraft text-sm font-bold uppercase tracking-wide text-theme/70">Данные версии</h4>
            ...
            <textarea className="min-h-[120px] w-full rounded border border-white/10 bg-black/20 p-2 font-mono text-sm text-theme" value={p.changelogChangesInput} onChange={(e) => p.setChangelogChangesInput(e.target.value)} placeholder={stringifyChangelogChanges(["- change 1"])} />
            ...
            <div className="mt-6 flex justify-end gap-3 border-t-2 border-theme/20 pt-6">
              <ShadcnButton 
                variant="secondary" 
                onClick={() => { p.resetChangelogForm(); p.setChangelogTab("all"); }}
                size="default"
              >
                ✕ Отмена
              </ShadcnButton>
              <ShadcnButton 
                ...
                onClick={p.editingChangelog ? p.handleUpdateChangelog : p.handleCreateChangelog}
                size="default"
