    ...
    <div className={cn("flex flex-col gap-1", className)}>
      ...
      <div className="relative">
        <input
          ...
          type={effectiveType}
          className={cn(
            "mc-input text-base transition-colors duration-75 disabled:cursor-not-allowed disabled:opacity-50",
            isPassword && "pr-12",
          )}
          {...props}
        />
        ...
        {isPassword && (
          <button
            ...
            className="absolute right-3 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center text-theme-muted transition-colors hover:text-theme focus:outline-none"
      ...
      {error && (
        <span className="mt-1 font-minecraft text-xs text-[var(--mc-error-text)]">
          {error}
        </span>
      )}
    </div>
