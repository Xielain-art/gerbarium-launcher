...
export function DashboardScreen() {
  ...
  const { isDownloading, progress, startDownload, cancelDownload } = useDownloadStore();
      ...
      <aside className="relative z-40 flex h-full w-80 flex-col border-r-[4px] border-[#1a1a1a] bg-[#1e2025]/95 backdrop-blur-md shadow-2xl">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-minecraft text-sm font-bold text-[#e0e0e0] truncate" title={user?.username || ''}>
                    {user?.username || 'Игрок'}
                  </span>
            ...
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="square" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
              </svg>
            </button>
          </div>
        </div>
        ...
        {serverStatus && (
          <div className="border-b-[3px] border-[#1a1a1a] bg-[#25272c]/50 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className={`h-2.5 w-2.5 rounded-full ${serverStatus.online ? 'bg-[#55ff55] animate-pulse' : 'bg-[#ff5555]'}`} />
        ...
        <div className="border-t-[3px] border-[#1a1a1a] bg-[#2b2d31]/95 p-4">
          <button
            ...
            className="mc-btn mc-btn-md w-full"
          >
                ...
                news.map((item) => (
                  ...
                  >
                    ...
                    <div className="mb-3 flex items-center gap-3">
                      <span className={`px-3 py-1 font-minecraft text-xs font-bold text-[#e0e0e0] ${getCategoryColor(item.category)}`}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4 min-w-0">
                <div className="min-w-0">
                  ...
                  <div className="font-minecraft text-xs">
                    {mockVersions.find((v) => v.id === selectedVersion)?.isInstalled 
                      ? <span className="text-[#5a5]">✓ Готово к запуску</span>
                      : <span className="text-[#8a8a8a]">○ Требуется установка</span>
                    }
                  </div>
                </div>
              </div>
