...
## Источник истины
Источник истины: удаленный `packs/client/pack.toml` (через env `PACKWIZ_PACK_URL`).
...
Лаунчеру нужен доступ к `pack.toml` по env `PACKWIZ_PACK_URL`.
...
Что делать:
1. Проверить env `PACKWIZ_PACK_URL`.
